...
;( function( $, window, undefined ) {	
	...
	$.Calendario.defaults = {
		...
		startIn: 1, // 1 = Lunes como primer día de la semana
...
var eventos = {
  ...
  var cal = $('#calendar').calendario({
    caldata: eventos,
    onDayClick: function ($el, $content, dateProperties) {
      // Normalizar fecha con ceros a la izquierda
      var day = String(dateProperties.day).padStart(2, '0');
      var month = String(dateProperties.month).padStart(2, '0');
      var fechaClave = `${dateProperties.year}-${month}-${day}`;

      console.log("Fecha clickeada:", fechaClave);

      // Obtener contenido del evento
      var evento = eventos[fechaClave] || '<em>No hay eventos para este día.</em>';

      // Mostrar en modal
      var contenido = `
        <p><strong>Fecha seleccionada:</strong> ${day}/${month}/${dateProperties.year}</p>
        <p>${evento}</p>
      `;
